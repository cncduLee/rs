...
var pagerOptions = {};
var homeViewModel, queryViewModel;
var supportHistoryPushstate = true;
function initializeSearchInput() {
    $("#searchKeyWordInput").keyup(function (a) {
        if (a.keyCode == 13) {
            $("#searchKeyWordBtn").click()
        }
    });
}
var scrollTopSearchItinerary = 0;
function queryData(a) {
    $("#searchLoading").css({display: "block"});
    $.ajax({url: "/Itinerary/QueryItineraryInfo", type: "post", data: a, success: function (b) {
        InnTwoTools.JsonDataCallback({data: b, bussinessException: function () {
        }, success: function () {
            $("#searchKeyWordBtn").removeAttr("disabled").attr({value: "搜  索"});
            $("#searchLoading").hide();
            if (b && typeof b == "string") {
                $("#homeItineraryList").html(b);
                setPlaceResultCount();
                $(".empty-warning-out").hide()
            } else {
                $(".empty-warning-out").show()
            }
            lazyLoadInitial();
            showStrar();
            var c = $(".inntwo-home-search-outter").offset().top - 80;
            if (scrollTopSearchItinerary == 0) {
                scrollTopSearchItinerary = c
            } else {
                if (scrollTopSearchItinerary < c) {
                    scrollTopSearchItinerary++
                } else {
                    scrollTopSearchItinerary--
                }
            }
            $("html,body").animate({scrollTop: scrollTopSearchItinerary}, 100)
        }})
    }})
}
var travelThem = {startSelectThem: function () {
    $("#itinerariesPlacesCategoriesOuter a:first").tab("show");
    $("#itinerariesPlacesCategoriesOuter a:first").css({border: "none", "background-color": "#4da49f", color: "white"});
    $("#itinerariesPlacesCategoriesOuter .active a").css({"border-bottom": "4px solid rgba(255, 100, 0, 0.7)"});
    var a = $("#hotDisCategory0").children();
    for (var b = 0; b < a.length; b++) {
        var c = a.eq(b).children("img");
        c.attr("src", c.attr("data-index"))
    }
}, travelThemClick: function () {
    $("#itinerariesPlacesCategoriesOuter a").click(function (c) {
        c.preventDefault();
        $("#itinerariesPlacesCategoriesOuter").children().css({border: "none"});
        $(".itineraries-places-categories a").css({"border-bottom": "none"});
        $(this).css({"border-bottom": "4px solid rgba(255, 100, 0, 0.7)"});
        $(this).tab("show");
        var a = $(this).attr("data");
        var g = travelThem.market;
        if (a == g[0]) {
            travelThem.leftMove()
        }
        if (a == g[3]) {
            travelThem.rightMove()
        }
        var h = $(this).attr("href");
        var b = $(h).children();
        for (var d = 0; d < b.length; d++) {
            var f = b.eq(d).children("img");
            f.attr("src", f.attr("data-index"))
        }
    })
}, itinerariesArrowsLeftClick: function () {
    $("#itinerariesArrowsLeft").click(function () {
        travelThem.leftMove()
    })
}, itinerariesArrowsRightClick: function () {
    $("#itinerariesArrowsRight").click(function () {
        travelThem.rightMove()
    })
}, leftMove: function () {
    var c = $("#itinerariesPlacesCategoriesOuter").children();
    var b = travelThem.market;
    if (b[0] == 0) {
        return
    } else {
        for (var a = 0; a < 4; a++) {
            travelThem.market[a] = b[a] - 1
        }
        c.hide();
...
function searchBtnClick() {
    $("#searchKeyWordBtn").click(function () {
        var c = $.trim($("#searchKeyWordInput").val());
        var b = 13;
        if (c.length > b) {
            InnTwoTools.Alert("检索字数长度不能超过" + b + "个字!");
            return
        }
        $(this).attr({disabled: true, value: "检索中..."});
        var a = copyQueryModel(true);
        queryViewModel.KeyWord = a.KeyWord = c;
        querySubmit(a);
        if (c) {
            $("#itinerarySearchKeyword .itinerary-search-tags").text(c);
            $("#itinerarySearchKeyword").show();
            $("#itinerariesSelectedCategories").show()
        }
        $("#itinerarySearchKeyword .remove-search-keyword").attr("data", $("#searchKeyWordInput").val());
        itineraryCategorySelector.clearQuery()
    })
}
function copyQueryModel(b) {
    var a = {};
    a.OrginPlace = queryViewModel.OrginPlace;
    if (b) {
        a.Tags = "";
        a.Dest = "";
        a.KeyWord = "";
    ...
    a.Tags = queryViewModel.Tags;
    a.Dest = queryViewModel.Dest;
    a.KeyWord = queryViewModel.KeyWord;
    a.Days = queryViewModel.Days;
    a.PageIndex = queryViewModel.PageIndex;
    a.Seasons = queryViewModel.Seasons;
    a.Sort = queryViewModel.Sort;
    return a
}
function querySubmit(b) {
    var a = queryViewModel;
    if (undefined !== b) {
        a = b
    }
    if (a && a.KeyWord) {
        $("#searchKeyWordInput").val(a.KeyWord)
    }
    queryData(a);
    queryViewModel.PageIndex = 1
}
function setInterfaceSelected() {
    $("#inntwoHomeTripDays .days-row a").eq(0).css({"background-color": "#00A1CB", color: "white"});
    $("#inntwoQueryTripSeansonContainer .days-row a").eq(0).css({"background-color": "#CC0000", color: "#fff"});
    var a = $.trim($("#selectedDes").val());
    if (a) {
        queryViewModel.Dest = a;
        $("#inntwoQueryHotDesContainer .days-row a").each(function () {
            if ($.trim($(this).text()) == a) {
                $(this).css({"background-color": "#FF6600", color: "white"});
                return
            }
        })
    } else {
        $("#inntwoQueryHotDesContainer .days-row a").eq(0).css({"background-color": "#FF6600", color: "white"})
    }
    var b = $.trim($("#selectedTag").val());
    if (b) {
        $("#inntwoQueryTripThemeContainer .days-row a").each(function () {
            if ($.trim($(this).text()) == b) {
                $(this).css({"background-color": "#7DB500", color: "white"});
                queryViewModel.Tags = $(this).attr("topicseqhash");
                return
            }
        })
    } else {
        $("#inntwoQueryTripThemeContainer .days-row a").eq(0).css({"background-color": "#7DB500", color: "white"})
    }
}
function cityChoise() {
    $(".inntwo-home-start-choise").click(function () {
        $(".city-choise-items").show()
    });
    $(".city-choise-close-button").click(function () {
        $(".city-choise-items").hide()
    });
    $(".inntwo-home-start-choise").mouseleave(function () {
        $(".city-choise-items").hide()
    })
}
function pagerOnBegoin() {
    $("html,body").animate({scrollTop: $(".inntwo-home-search-outter").offset().top - 80}, 100);
    $("#searchLoading").css({display: "block"})
}
function pagerOnComplete() {
    $("#searchLoading").hide();
    showStrar();
    lazyLoadInitial();
    $("html,body").animate({scrollTop: $(".inntwo-home-search-outter").offset().top - 70}, 100)
}
function setPlaceResultCount() {
    $(".itineraries-query-count").show();
    $(".itineraries-query-count span").text($("#itineraryQueryResult").attr("dataCount"))
}
function lazyLoadInitial() {
    $(".inntwo-home-itinerary-img").lazyload({effect: "fadeIn"})
}
var itineraryCategorySelector = {topicAClick: function () {
    $("#inntwoQueryTripThemeContainer .days-row a").click(function () {
        queryViewModel.Tags = $(this).attr("topicseqhash");
        querySubmit();
        $("#inntwoQueryTripThemeContainer .days-row a").css({"background-color": "#fff", color: "#333"});
        $(this).css({"background-color": "#7DB500", color: "white"});
        var b = $(this).text().trim();
        var a = $("#itineraryQueryTheme .itinerary-search-tags");
        if (b == "不限") {
            $("#itineraryQueryTheme").hide();
            $("#itineraryQueryTheme .remove-search-keyword").attr("data", "0");
            itineraryCategorySelector.isCheckRemove()
        } else {
            a.text(b);
            $("#itineraryQueryTheme").show();
            $("#itinerariesSelectedCategories").show();
            $("#itineraryQueryTheme .remove-search-keyword").attr("data", "topic")
        }
        return false
    })
}, daysAClick: function () {
    $("#inntwoHomeTripDays .days-row a").click(function () {
        var b = $(this).attr("data");
        queryViewModel.Days = (b == "0" ? "" : b);
        querySubmit();
        $("#inntwoHomeTripDays .days-row a").css({"background-color": "#fff", color: "#333"});
        $(this).css({"background-color": "#00A1CB", color: "white"});
        var c = $(this).text().trim();
        var a = $("#itineraryQueryDays .itinerary-search-tags");
        if (c == "不限") {
            $("#itineraryQueryDays").hide();
            $("#itineraryQueryDays .remove-search-keyword").attr("data", "0");
            itineraryCategorySelector.isCheckRemove()
        } else {
            a.text(c);
            $("#itineraryQueryDays").show();
            $("#itinerariesSelectedCategories").show();
            $("#itineraryQueryDays .remove-search-keyword").attr("data", "day")
        }
        return false
    })
}, seasonAClick: function () {
    var a = $("#inntwoQueryTripSeansonContainer .days-row a");
    a.click(function () {
        a.css({"background-color": "#fff", color: "#333"});
        ...
        queryViewModel.Seasons = c;
        querySubmit();
        var e = $(this).text().trim();
        var d = $("#itineraryQuerySeason .itinerary-search-tags");
        if (e == "不限") {
            $("#itineraryQuerySeason").hide();
            $("#itineraryQuerySeason .remove-search-keyword").attr("data", "0");
            itineraryCategorySelector.isCheckRemove()
        } else {
            d.text(e);
            $("#itineraryQuerySeason").show();
            $("#itinerariesSelectedCategories").show();
            $("#itineraryQuerySeason .remove-search-keyword").attr("data", "season")
        }
        return false
    })
}, removeItineraryCategory: function () {
    $("#itinerarySearchKeyword .remove-search-keyword").click(function () {
        $(this).parent().hide();
        $(this).attr("data", "");
        $("#itinerarySearchKeyword .remove-search-keyword").attr("data", "");
        itineraryCategorySelector.isCheckRemove();
        queryViewModel.KeyWord = "";
        querySubmit()
    });
    $("#itineraryQueryDays .remove-search-keyword").click(function () {
        $(this).parent().hide();
        $(this).attr("data", "0");
        var a = $("#inntwoHomeTripDays .days-row a");
        a.first().click()
    });
    $("#itineraryQuerySeason .remove-search-keyword").click(function () {
        $(this).parent().hide();
        $(this).attr("data", "0");
        var a = $("#inntwoQueryTripSeansonContainer .days-row a");
        a.first().click()
    });
    $("#itineraryQueryTheme .remove-search-keyword").click(function () {
        $(this).parent().hide();
        $(this).attr("data", "0");
        var a = $("#inntwoQueryTripThemeTitle a");
        a.first().click()
    })
}, clearQuery: function () {
    itineraryCategorySelector.isCheckRemove()
}, isCheckRemove: function () {
    if ($("#itineraryQuerySeason .remove-search-keyword").attr("data") == "0" && $("#itineraryQueryDays .remove-search-keyword").attr("data") == "0" && $("#itineraryQueryTheme .remove-search-keyword").attr("data") == "0" && $("#itinerarySearchKeyword .remove-search-keyword").attr("data") == "") {
        $("#itinerariesSelectedCategories").hide();
        return
    }
}, itinerariesSortPrice: function () {
    $("#itinerariesSortPrice").click(function () {
        itineraryCategorySelector.strPrice = "";
        var a = $("#itinerariesSortPriceIco");
        if (itineraryCategorySelector.countPrice % 2 == 0) {
            itineraryCategorySelector.strPrice = "amount:asc";
            a.removeClass("itineraries-sort-price-up");
            a.addClass("itineraries-sort-price-down")
        } else {
            itineraryCategorySelector.strPrice = "amount:desc";
            a.removeClass("itineraries-sort-price-down");
            a.addClass("itineraries-sort-price-up")
        }
        queryViewModel.SortOrders = itineraryCategorySelector.strPrice;
        querySubmit();
        a.show();
        $("#itinerariesSortDayIco").hide();
        $("#itinerariesSortPrice .itineraries-sort-cell-title").removeClass("itineraries-sort-cell-title");
        $("#itinerariesSortDay span").addClass("itineraries-sort-cell-title");
        itineraryCategorySelector.countPrice++
    })
}, itinerariesSortDay: function () {
    $("#itinerariesSortDay").click(function () {
        itineraryCategorySelector.strDay = "";
        var a = $("#itinerariesSortDayIco");
        if (itineraryCategorySelector.countDay % 2 == 0) {
            itineraryCategorySelector.strDay = "daytotal:asc";
            a.removeClass("itineraries-sort-price-up");
            a.addClass("itineraries-sort-price-down")
        } else {
            itineraryCategorySelector.strDay = "daytotal:desc";
            a.removeClass("itineraries-sort-price-down");
            a.addClass("itineraries-sort-price-up")
        }
        queryViewModel.SortOrders = itineraryCategorySelector.strDay;
        querySubmit();
        $("#itinerariesSortPriceIco").hide();
        a.show();
        $("#itinerariesSortDay .itineraries-sort-cell-title").removeClass("itineraries-sort-cell-title");
        $("#itinerariesSortPrice span").addClass("itineraries-sort-cell-title");
        itineraryCategorySelector.countDay++
    })
}, countPrice: 0, countDay: 0, strDay: "", strPrice: ""};
var banner = {counter: 0, boolStop: true, itinerariesBannerPlay: function () {
    var b = $("#itinerariesBannerContentContainerLeft").children();
    for (var a = 0; a < b.length; a++) {
        b.eq(a).css({"background-color": "rgba(255,255,255,0.5)"});
        $(".text-color").css("color", "rgba(51,51,51,0.9)")
    }
    $(".itineraries-banner-content-container-right").hide();
    if (banner.counter == b.length) {
        $("#itinerariesBannerContentContainerRight0").show();
        banner.counter = -1;
        b.eq(0).css({"background-color": "rgba(51,51,51,0.5)"});
        $("#itinerariesBanner0 .text-color").css("color", "rgba(255,255,255,1)");
        return
    }
    b.eq(banner.counter).css({"background-color": "rgba(51,51,51,0.5)"});
    $("#itinerariesBanner" + banner.counter + " .text-color").css("color", "rgba(255,255,255,1)");
    banner.bannerImages();
    if (banner.counter == 2) {
        banner.counter = -1
    }
}, bannerSetTime: function () {
    setInterval(function () {
        banner.bannerImage()
    }, 5000)
}, itinerariesBannerContentClick: function () {
    $(".itineraries-banner-fillet").click(function () {
        banner.counter = $(this).attr("data");
        banner.itinerariesBannerPlay();
        banner.boolStop = false;
        banner.bannerImages()
    })
    ...
    if (banner.counter == 1) {
        $("#itinerariesBannerContentContainerRight1").show()
    }
    if (banner.counter == 2) {
        banner.counter = -1;
        $("#itinerariesBannerContentContainerRight2").show()
    }
}, bannerImagesStop: function () {
    $(".itineraries-banner-fillet").hover(function () {
        var a = $("#itinerariesBannerContentContainerLeft").children();
        if (banner.counter == -1) {
            banner.counter = a.length - 1
        }
        if (banner.counter == $(this).attr("data")) {
            banner.boolStop = false
        }
    }, function () {
        banner.boolStop = true;
        var a = $("#itinerariesBannerContentContainerLeft").children();
        if (banner.counter == a.length - 1) {
            banner.counter = -1
        }
    });
    $("#itinerariesBannerConntainerPhoto").hover(function () {
        banner.boolStop = false
    }, function () {
        banner.boolStop = true
    });
    $("#itinerariesBannerContentContainerRight0").hover(function () {
        banner.boolStop = false
    }, function () {
        banner.boolStop = true
    });
    $("#itinerariesBannerContentContainerRight1").hover(function () {
        banner.boolStop = false
    }, function () {
        banner.boolStop = true
    });
    $("#itinerariesBannerContentContainerRight2").hover(function () {
        banner.boolStop = false
    }, function () {
        banner.boolStop = true
    })
}, bannerImage: function () {
    if (banner.boolStop == true) {
        banner.counter++;
        banner.itinerariesBannerPlay(banner.counter)
    }
}};
var hotDestination = {deleteDestination: function () {
    $(".element-remove").click(function () {
        $(this).parent().remove();
        var a = $(this).attr("point");
        $(".itineraries-city-add").each(function () {
            if (a == $(this).attr("point")) {
                $(this).show()
            }
        })
    })
}, clickedPlaceAdd: null, destinationFavorite: function () {
    $(".itineraries-city-add").click(function () {
        if ($(".itineraries-favorite-places").length >= 6) {
            return
        } else {
            var c = $(this).prevAll("img");
            var d = c.offset();
            var a = $("#homePlacesBoxIco").offset();
            var b = "<img class='itineraries-favorite-animate' src='" + c.attr("src") + "' />";
            hotDestination.clickedPlaceAdd = $(this);
            $("body").append(b);
            $(".itineraries-favorite-animate").css({left: d.left + "px", top: d.top + "px"});
            $(".itineraries-favorite-animate").animate({left: (a.left + 5) + "px", top: (a.top + 20) + "px", width: 20 + "px", height: 20 + "px"}, 400, function () {
                $(".itineraries-favorite-animate").fadeOut();
                hotDestination.places.push({src: hotDestination.clickedPlaceAdd.prevAll("img").attr("src"), point: hotDestination.clickedPlaceAdd.attr("point"), placeid: hotDestination.clickedPlaceAdd.attr("placeid"), placeName: hotDestination.clickedPlaceAdd.attr("placeName"), duration: hotDestination.clickedPlaceAdd.attr("duration")});
                hotDestination.clickedPlaceAdd = null;
                $(".itineraries-favorite-animate").remove();
                $(".itineraries-favorite-places").hover(function () {
                    $(this).children(".element-remove").show()
                }, function () {
                    $(this).children(".element-remove").hide()
                });
                hotDestination.deleteDestination()
            });
            $("#itinerariesdestinationFavorite").hide();
            $(this).hide()
        }
    })
}, places: ko.observableArray([]), planingItinerary: function () {
    $("#generationItinerary").click(function () {
        var c = hotDestination.collectPlaces();
        if (c.OrginPlace == "" || c.OrginPlace == "all" || c.OrginLocation == "" || typeof c.OrginPlace === "undefined" || typeof c.OrginLocation == "undefined") {
            var a = c.PlanningTripDestinations[0];
            if (a) {
                c.OrginLocation = InnTwoTools.ConvertCoor(a.DestinationLocation);
                c.OrginPlace = a.DestinationName
            }
        }
        if (!c.OrginPlace || c.PlanningTripDestinations.length < 1) {
            InnTwoTools.Alert("请选择景点！");
            return
        }
        var b = JSON.stringify(c);
        window.localStorage.setItem("home.planingPlaces", b);
        window.open("/createitinerary/console?homeredirect=" + InnTwoTools.getIdentifyStringByDate())
    })
}, collectPlaces: function () {
    var a = {};
    var b = [];
    $(".element-remove").each(function () {
        var e = $(this).attr("placeid");
        var g = $(this).attr("point");
        var f = $(this).attr("placeName");
        var d = $(this).attr("duration");
        var c = {DestinationId: e, DestinationLocation: g, DestinationName: f, Duration: d};
        b.push(c)
    });
    a.PlanningTripDestinations = b;
    a.OrginLocation = $(".itineraries-content-right-title").attr("orignlocation");
    a.OrginPlace = $(".itineraries-content-right-title").attr("orignname");
    return a
}, };
function mainHref() {
...
function showStrar() {
    $(".itinerary-star").each(function () {
        var a = $(this).attr("data-score");
        if (a) {
            $(this).raty({readOnly: true, score: a, half: true, hints: ["糟糕透了", "不理想", "一般般", "很好", "棒极了"], path: "/static/images/asset2"})
        }
    })
}
function controlHomeDownload() {
    $(window).scroll(function () {
        var a = $(this).scrollTop();
        if (a > 1230) {
            $(".home-app-download-background").css({position: "fixed"})
        } else {
            $(".home-app-download-background").css({position: "static"})
        }
    })
}
$(function () {
    ...
    $("#mainInntwoMenuIco").click(function () {
        mainHref();
        return false
    });
    itineraryCategorySelector.itinerariesSortDay();
    itineraryCategorySelector.itinerariesSortPrice();
    travelThem.itinerariesArrowsLeftClick();
    travelThem.itinerariesArrowsRightClick();
    travelThem.startSelectThem();
    travelThem.travelThemClick();
    banner.itinerariesBannerContentClick();
    banner.bannerSetTime();
    banner.itinerariesBannerPlay();
    banner.bannerImagesStop();
    supportHistoryPushstate = (typeof window.history.pushState !== "undefined");
    itineraryCategorySelector.daysAClick();
    itineraryCategorySelector.topicAClick();
    itineraryCategorySelector.seasonAClick();
    itineraryCategorySelector.removeItineraryCategory();
    searchBtnClick();
    queryViewModel = {};
    queryViewModel.OrginPlace = $(".orign-city").text().trim();
    queryViewModel.OrginPlace = queryViewModel.OrginPlace == "不限" ? "" : queryViewModel.OrginPlace;
    queryViewModel.Tags = "";
    queryViewModel.Dest = "";
    queryViewModel.KeyWord = "";
    queryViewModel.Days = "";
    queryViewModel.PageIndex = 1;
    queryViewModel.Seasons = "";
    setInterfaceSelected();
    initializeSearchInput();
    cityChoise();
    lazyLoadInitial();
    $(".carousel").carousel();
    if ($(".inntwo-home-itinerary-item ,.inntwo-home-activity-item").length < 1) {
        $(".empty-warning-out").show()
    }
    window.inntwoBase.goToTop(300);
    window.InnTwoTools.stickUp(".create_route_top", 0, 0, 2, 0, function () {
        $(".cret_rout_content").css({"margin-top": "72px"})
    }, function () {
        $(".cret_rout_content").css({"margin-top": "0"})
    });
    window.inntwoBase.setSelectedMenuStyle(0);
    setPlaceResultCount();
    controlHomeDownload()
});